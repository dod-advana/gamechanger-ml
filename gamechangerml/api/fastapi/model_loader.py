...
class ModelLoader:
    ...
    def getQENgram(self):
        if ModelLoader.__query_expander_ngram == None:
            logger.warning(
                "query_expander was not set and was attempted to be used. Running init"
            )
            ModelLoader.initQENGram()
        return ModelLoader.__query_expander_ngram
    ...
    query_expander_ngram = property(getQENgram, set_error)
    ...
    @staticmethod
    def initQENGram(qexp_ngram_model_path=QEXP_NGRAM_MODEL_PATH_LIST.value):
        """initQENGram - loads NGRAM QE models on start
        Args:
        Returns:
        """
        qexp_ngram_model_path = qexp_ngram_model_path.split(",")
        logger.info(f"Loading Pretrained Vector from {qexp_ngram_model_path}")
        ngram_regex = "ngram_(\d)_(\d)"
        try:
            ngram_qexp_dict = {}
            for qexp_ngram_model in qexp_ngram_model_path:
                match = re.search(ngram_regex,qexp_ngram_model)
                if match:
                    ngram_key = f"({match.groups()[0]},{match.groups()[1]})"
                    if ngram_key in ngram_qexp_dict:
                        logger.info(f"skipping loading {qexp_ngram_model} due to a more recent {ngram_key} model being loaded in")
                        continue
                    ngram_qexp_dict[ngram_key] = qe.QE(
                        qexp_ngram_model, **QexpConfig.MODEL_ARGS["init"]
                    )
                    logger.info(f"Loaded in {ngram_key} ngram model: {qexp_ngram_model}")
                else:
                    logger.info(f"Unable to regex match {qexp_ngram_model}, skipping this ngram model")
            ModelLoader.__query_expander_ngram = ngram_qexp_dict
            logger.info(f"** Loaded {len(ngram_qexp_dict)} total JBOOK n-gram Query Expansion Model")
        except Exception as e:
            logger.warning("** Could not load GC Core n-gram QE model")
            logger.warning(e)
