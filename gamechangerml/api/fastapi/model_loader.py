...
class ModelLoader:
    ...
    def getSemanticSearcher(self):
        if ModelLoader.__semantic_searcher == None:
            logger.warning(
    ...
    @staticmethod
    def initSemanticSearcher(
        index_path=TITLE_INDEX_PATH.value, transformer_path=LOCAL_TRANSFORMERS_DIR.value
    ):
        """
        initSemanticSearcher - loads SemanticSearcher class on start
        Args: 
            index_path (str): Path to index directory generated by the
            SentenceEncoder
            transformer_path (str): Path to the model used to generate the document embeddings
        Returns:
        """
        logger.info(
            f"Loading Semantic Searcher with sentence index path: {index_path}")
        try:
            # assign a new instance of the SemanticSearcher 
            ModelLoader.__semantic_searcher = SemanticSearcher(
                sim_model_name=SemanticConfig.BASE_MODEL,
                index_path=index_path,
                transformer_path=transformer_path,
            )
            # assign the similarity attribute of the __semantic_searcher instance to the sim_model variable
            sim_model = ModelLoader.__semantic_searcher.similarity
            # set cache variable defined in settings.py
            latest_intel_model_sim.value = sim_model.sim_model
            logger.info(
                f"** Loaded Similarity Model from {sim_model.sim_model} and title index from {index_path}"
            )
