...
function setup_prod() {
    ...
    export S3_TITLE_INDEX_PATH="${S3_TITLE_INDEX_PATH:-s3://advana-data-zone/bronze/gamechanger/models/title_index/v4/title_index_20230605.tar.gz}"
...
function setup_dev() {
    ...
    export S3_TRANS_MODEL_PATH="${S3_TRANS_MODEL_PATH:-s3://advana-data-zone/bronze/gamechanger/models/transformers/v1/transformers.tar.gz}"
    export S3_SENT_INDEX_PATH="${S3_SENT_INDEX_PATH:-s3://advana-data-zone/bronze/gamechanger/models/sentence_index/v4/sentence_index_20221208.tar.gz}"
    export S3_TITLE_INDEX_PATH="${S3_TITLE_INDEX_PATH:-s3://advana-data-zone/bronze/gamechanger/models/title_index/v4/title_index_20230605.tar.gz}"
    export S3_QEXP_PATH="${S3_QEXP_PATH:-s3://advana-data-zone/bronze/gamechanger/models/qexp_model/v4/qexp_20221209.tar.gz}"
    export S3_QEXP_JBOOK_PATH="${S3_QEXP_JBOOK_PATH:-s3://advana-data-zone/bronze/gamechanger/models/jbook_qexp_model/v2/jbook_qexp_20220131.tar.gz}"
    export S3_TOPICS_PATH="${S3_TOPICS_PATH:-s3://advana-data-zone/bronze/gamechanger/models/topic_model/v4/topic_model_20221208211047.tar.gz}"
    export S3_ML_DATA_PATH="${S3_ML_DATA_PATH:-s3://advana-data-zone/bronze/gamechanger/ml-data/v1/data_20220127.tar.gz}"
    export S3_CORPUS_PATH="${S3_CORPUS_PATH:-s3://advana-data-zone/bronze/gamechanger/json}"
    export LOCAL_CORPUS_PATH="${LOCAL_CORPUS_PATH:-$PWD/gamechangerml/corpus}"
    export DEV_ENV="DEV"
...
cat <<EOF
  ...
  * S3_TITLE_INDEX_PATH: ${S3_TITLE_INDEX_PATH:-<unset>}"
  ...
  * CONTAINER_TYPE: ${CONTAINER_TYPE:-<unset>}"
