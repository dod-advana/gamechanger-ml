...
def get_best_paragraphs(
    data: pd.DataFrame, query: str, doc_id: str, nlp, min_score: float = 0.60
) -> List[Dict[str, str]]:
    ...
    try:
        if len(sents) == 0:
            ...
            ranked = [{"score": "na", "id": sents[0]
                       ["id"], "text": sents[0]["text"]}]
        else:
            comparisons = []
            for sent in sents:
                doc2 = nlp(sent["text"])
        ...
        logger.info(
            f"*** Collected {str(len(ranked))} / {str(len(sents))} paragraphs (passing sim threshold) retrieved for {doc_id}"
        )
...
def check_no_match(expected_id: str, par_id: str) -> bool:
    ...
    if (
        par_id.split(".pdf")[0].upper().strip().lstrip()
        == expected_id.upper().strip().lstrip()
    ):
        return False
    else:
        return True
...
def get_negative_paragraphs(
    data: pd.DataFrame, query: str, doc_id: str, retriever, n_returns: int
) -> List[Dict[str, str]]:
        ...
        for result in results:
            ...
            if check_no_match(doc_id, result["id"]):
                checked_results.append(
                    {"query": query,
                        "doc": result["id"], "paragraph": par, "label": 0}
                )
    except Exception as e:
...
def make_training_data(
    ...
    incorrect_train, incorrect_test = train_test_split(
        incorrect_found, tts_ratio)
...
if __name__ == "__main__":
    ...
    make_training_data(
        index_path="gamechangerml/models/sent_index_20210715",
        n_returns=50,
        n_matching=3,
        level="silver",
