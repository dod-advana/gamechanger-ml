...
SIM_MODEL = SimilarityRankerConfig.BASE_MODEL_NAME
    ...
    if resp["hits"]["total"]["value"] > 0:
        ...
        for doc in docs:
            ...
            for par in hits:
                ...
                processed = " ".join(
                    simple_preprocess(par_text, min_len=2, max_len=100)
                )
...
def format_nonmatching_paragraphs(
    ...
    try:
        ...
        for r in results:
            if len(found) >= (
                par_count * 4
            ):  # stop getting negatives after 1:4 ratio
...
def get_any_matches(any_matches, matching_docs, query):
    ...
    try:
        single_matching_docs = [
            clean_id(i)
            for i in any_matches[query]
            if clean_id(i) not in matching_docs
        ]
        logger.info(
...
def collect_paragraphs_es(
    correct, incorrect, queries, collection, any_matches
):
    ...
    for i in correct.keys():
        ...
        for k in correct[
            i
        ]:  # for each possible match, collect positive samples
...
def train_test_split(
    data: Dict[str, str], tts_ratio: float
) -> Tuple[Dict[str, str]]:
...
def make_training_data_csv(data, label):
    ...
    agg_match["num_matching_paragraphs"] = agg_match[
        "matching_paragraphs"
    ...
    agg_nonmatch["num_nonmatching_paragraphs"] = agg_nonmatch[
        "nonmatching_paragraphs"
    ].apply(lambda x: len(x))
    agg_nonmatch["num_nonmatching_docs"] = agg_nonmatch[
        "nonmatching_paragraphs"
    ].apply(lambda x: count_unique(x))
    ...
    combined["par_balance"] = (
        combined["num_matching_paragraphs"]
        / combined["num_nonmatching_paragraphs"]
    )
    combined["doc_balance"] = (
        combined["num_matching_docs"] / combined["num_nonmatching_docs"]
    )
