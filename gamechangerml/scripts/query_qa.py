...
def format_search_context(search_results, context_type="all"):
    ...
    regular_context = [
        {
            "text": i["pageHits"][0]["snippet"].replace("<em>|</em>", ""),
            "source": i["display_title_s"],
            "type": "regular",
        }
        for i in search_results["saveResults"]["regular"]
    ]
    qa_context = [
        {"text": i["text"], "source": i["filename"], "type": "qa_context"}
        for i in search_results["saveResults"]["context"]
    ]

    if context_type == "all":
        context = dict(
            zip(
                range(len(regular_context) + len(qa_context)),
                qa_context + regular_context,
            )
        )
...
class QAResponse:
    ...
    def send_qa(self):
        ...
        timer = Timer()
        with timer:
            post = {"query": self.query, "search_context": self.simple_context}
        ...
        took = f"{timer.elapsed:0.4f} seconds"
...
if __name__ == "__main__":
    ...
    results_csv.write(
        "question,answer,score,prob,filename,context_rank,answer_rank,source"
    )
