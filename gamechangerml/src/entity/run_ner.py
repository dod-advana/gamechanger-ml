...
from gamechangerml.src.entity.utils_ner import (
    Split,
...
class DataTrainingArguments:
    ...
    data_dir: str = field(
        metadata={
            "help": "The input data dir. Should contain the .txt files for a "
            "CoNLL-2003-formatted task."
        }
    )
    labels: Optional[str] = field(
        default=None,
        metadata={
            "help": "Path to a file containing all labels. If not specified, "
            "CoNLL-2003 labels are used."
        },
    )
...
def main():
    ...
    if (
        ...
        raise ValueError(
            ...
            f" is not empty. Use --overwrite_output_dir to overcome."
        )
    ...
    try:
        ...
        raise ValueError(
            f"Task {model_args.task_type} needs to be defined as a "
            f"TokenClassificationTask subclass in {module}. "
            f"Available tasks classes "
            f"are: {TokenClassificationTask.__subclasses__()}"
        )
    ...
    # Set the verbosity to info of the Transformers logger (on main
    # process only):
