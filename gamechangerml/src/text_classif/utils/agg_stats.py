...
def linkage_report(
    ...
    """
    grouped_data = (
        df.groupby(["Source Document", "Organization / Personnel"])
        .count()
        .reset_index()
    )
    ...
    for i, row in grouped_data.iterrows():
        if row["Organization / Personnel"] == no_entity_id:
            output_data[row["Source Document"]][1] += row[
                "Responsibility Text"
            ]
        else:
            output_data[row["Source Document"]][0] += row[
                "Responsibility Text"
            ]

    output_df = pd.DataFrame.from_dict(
        output_data, orient="index"
    ).reset_index()
    output_df.rename(
        columns={"index": "Source Document", 0: "linked", 1: "unlinked"},
        inplace=True,
    )
    output_df["link_percent"] = round(
        (output_df["linked"])
        / (output_df["linked"] + output_df["unlinked"])
        * 100,
        2,
    )
    return output_df
...
if __name__ == "__main__":
    ...
    output_df = linkage_report(final_df)
