...
def expand_abbreviations(text, dic: t.Union[t.Dict[str, t.Any], str] = "gamechangerml/src/featurization/data/abbreviations.json"):
    """
            ...
            if key in text:
                for value in lower[key]:
                    ...
                    if text.lower().find(val.lower()) != -1:
                        ...
                        realtext = re.sub(rf"(?<!-)\b{re.escape(key)}\b(?!-)", value, realtext)
                        realtext = re.sub(
                            val + "[\,\.]?\s+" + val,
                            value,
                            realtext,
            ...
            if word[0].isupper() or word in dic.keys():
                for value in lower[word.lower()]:
                    ...
                    if text.lower().find(val.lower()) != -1:
                        d[word.lower()] = val
                        #regex identifies where the abbreviaiton is and prevents finding abbreviations within words
                        realtext = re.sub(rf"(?<!-)\b{re.escape(word)}\b(?!-)", value, realtext)
                        realtext = re.sub(
                            val + "[\,\.]?\s+" + val,
                            value,
                            realtext,
...
def expand_abbreviations_no_context(text, dic: t.Union[t.Dict[str, t.Any], str] = "gamechangerml/src/featurization/data/abbcounts.json"):
    """
...
def find_abbreviations(text, dic: t.Union[t.Dict[str, t.Any], str] = "gamechangerml/src/featurization/data/abbreviations.json"):
    """
    ...
    words = list(dic.keys())
    #characters between ^ and ] are the ones allowed by this regex
    words = [re.sub(pattern=r"""[^0-9a-zA-Z_ '"-]""", repl="", string=x) for x in words]
    words = [x.strip(' ').lower() for x in words]
    clean = {}
    for key in dic.keys():
        #characters between ^ and ] are the ones allowed by this regex
        k = re.sub(pattern=r"""[^0-9a-zA-Z_ '"-]""", repl="", string=key).strip(' ').lower()
        if k in clean:
            clean[k].append(key)
        else:
            clean[k] = [key]
    ...
    d = {key: None for key in words}
    #characters between ^ and ] are the ones allowed by this regex
    lower = {re.sub(pattern=r"""[^0-9a-zA-Z_ '"-]""", repl="", string=k).strip(' ').lower():v for k,v in dic.items()}
    for key in words:
        if " " in key:
            if key in text:
                for item in clean[key.lower()]:
                    for value in dic[item]:
                        #characters between ^ and ] are the ones allowed by this regex
                        val=re.sub(pattern=r"""[^0-9a-zA-Z_ &-'"]""", repl=" ", string=value)
                        val = re.sub(pattern=r"\s+", repl=r' ', string=val)
                        if text.lower().find(val.lower()) != -1:
                            #regex identifies where the abbreviaiton is and prevents finding abbreviations within words
                            num = len(re.findall(rf"(?<!-)\b{re.escape(key)}\b(?!-)",text))
                            d[item] = [value,num]
                            break
            ...
            if word[0].isupper() or word in dic.keys():
                for item in clean[word.lower()]:
                    for value in dic[item]:
                        ...
                        if text.lower().find(val.lower()) != -1:
                            #regex identifies where the abbreviaiton is and prevents finding abbreviations within words
                            num = len(re.findall(rf"(?<!-)\b{re.escape(word)}\b(?!-)",text))
                            d[item] = [value,num]
                            break
    d = dict((k, v) for k, v in d.items() if v)
    return d
