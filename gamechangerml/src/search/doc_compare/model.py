...
class DocCompareSentenceEncoder(object):
    """
...
class DocCompareSentenceSearcher(object):
    """
    ...
    def retrieve_topn(self, query, num_results, confidence_mapper=lambda *_: None) -> dict:
        ...
        for doc_id, score in retrieved:
            ...
            if confidence_mapper:
                doc["confidence"] = confidence_mapper(score)
    ...
    @staticmethod
    def confidence_mapper_creator(confidences) -> typing.Callable[[float], typing.Union[str, None]]:
        def mapper(score):
            try:
                for threshold, display in confidences:
                    if score < threshold:
                        return display
            except:
                return None
    ...
    CONFIDENCES = ((0.5, "Low"), (0.8, "Medium"), (2, "High"))
    ...
    def search(
        ...
        confidences = body.get(
            'confidences', DocCompareSentenceSearcher.CONFIDENCES)

        confidence_mapper = self.confidence_mapper_creator(confidences)

        if len(query) > 2:
            top_results = self.retrieve_topn(
                query, num_results, confidence_mapper)

            logger.info(f"top_results {top_results}")
            import time
            time.sleep(10)
